...
SELECT 
    L.leave_name,
    L.leave_Max_date, 
    IFNULL(SUM(DATEDIFF(lh.leavehis_end, lh.leavehis_start)), 0) AS used_leave_days,
    (L.leave_Max_date - IFNULL(SUM(DATEDIFF(lh.leavehis_end, lh.leavehis_start)), 0)) AS remaining_leave_days
FROM Leaves L
LEFT JOIN leave_his lh ON L.leave_id = lh.leave_id 
    AND lh.leave_status_id = 1  -- เฉพาะการลาที่ได้รับการอนุมัติ
    AND lh.emp_id = :user_id
