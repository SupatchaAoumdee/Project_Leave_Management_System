...
$sql_leaves = "SELECT leave_id, leave_name, leave_Max_date, leave_des FROM Leaves";
...
<head>
    ...
    <script>
        ...
        function validateDates() {
            ...
            const endDate = new Date(document.getElementById('endDate').value);
            const leaveType = document.getElementById('leaveType').value;
            const leaveTypeData = leaveTypes.find(type => type.leave_id == leaveType);
            ...
            if (leaveTypeData && leaveDays > leaveTypeData.leave_Max_date) {
                document.getElementById('error-message').innerText = `จำนวนวันที่เลือกเกินกว่ากำหนด (${leaveTypeData.leave_Max_date} วัน)`;
                return false;
            }
